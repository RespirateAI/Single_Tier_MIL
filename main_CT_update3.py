...
log_path = 'train_sample_log_update3_newL23232RLS.txt'
    ...
    def __call__(self, epoch, loss, error):
        ...
        if loss < self.best_loss:
            ...
            if self.model is not None:
                torch.save(self.model.state_dict(), "best_model_update3_newL2323RLS.pth")
        else:
            self.counter += 1
...
def train(epoch,callback=None):
    ...
    for batch in train_loader: 
        ...
        bag_probs = []
        bag_labels = []
        for (bag_id,tidx) in enumerate(index_chunk_list):
            ...
            print_log(f'{bag_id} BAG PRED {bag_prob}',log_file)
            bag_probs.append(bag_prob)
            bag_labels.append(tslideLabel)
        bag_labels = torch.Tensor(bag_labels).to('cuda') 
        bag_preds = torch.Tensor(bag_probs).to('cuda')   
        neg_log_likelihood = -1. * (bag_labels * torch.log(bag_preds) + (1. - bag_labels) * torch.log(1. - bag_preds))  # negative log bernoulli
        neg_log_likelihood.requires_grad = True
        # print("LOGLOSS",neg_log_likelihood)
        mean_loss = torch.mean(neg_log_likelihood)
        print_log(f'batch loss {mean_loss}',log_file)
