...
class MnistBags(data_utils.Dataset):
    ...
    def _create_bags(self):
        ...
        for i in range(self.num_bag):
            # generates a random number from a normal distribution with mean self.mean_bag_length and variance self.var_bag_length
            bag_length = np.int(self.r.normal(self.mean_bag_length, self.var_bag_length, 1))
            if bag_length < 1:
                bag_length = 1

            if self.train:
                # generates `bag_length` number of random integers between 0 (inclusive) and self.num_in_train (exclusive).
