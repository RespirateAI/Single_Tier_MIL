    ...
    def __call__(self, epoch, loss, error):
        ...
        if loss < self.best_loss:
            ...
            if self.model is not None:
                torch.save(self.model.state_dict(), "best_model_Image_Per_Bag.txt.pth")
        else:
            self.counter += 1
...
def train(epoch,callback=None):
    ...
    for batch in train_loader: 
        ...
        # reset gradients
        optimizer.zero_grad()
        loss, _ = model.calculate_objective(batch_feat[0], tslideLabel)
        train_loss += loss.data[0]
        error, _ = model.calculate_classification_error(batch_feat[0], tslideLabel)
        train_error += error
    ...
    log_line = f'Epoch: {epoch}, Loss: {train_loss.cpu().numpy()[0]}, Train error: {train_error}'
    print(log_line)
